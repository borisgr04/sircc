...
Public Class Cert_Contratos
    ...
    Public Overloads Function GetCertificadoD() As DataTable
        querystring = "Select Distinct c.Ide_Con, Contratista, nom_tip_ide,EXP_IDE,LPAD(Nro_Cert,5,0) NRO_CERT, Fec_Cert,nomusuario(UsAp) Elaboro,COD_SERIE,vig_cert,CASE WHEN c.tipoidentificacion = 'NI' THEN    trim(c.contratista)   || ' Con '  || c.nom_tip_ide  || ' '  || c.ide_con  || '-' || validardigitochequeonit (to_number(c.ide_con)) || ' Representada Legalmente por '   || c.rep_legal  || ' Identificado(a) con Cédula de Ciudadania ' || to_char(c.ide_rep,'999,999,999')  ELSE    trim(c.contratista)  || ' Identificado(a) con  '|| c.nom_tip_ide || ' N° '|| to_char(c.ide_con,'999,999,999') END contratista_c FROM vcontratos_sinc_p c,vCert_Contratos Where Vig_cert=To_Char(sysdate,'yyyy') And Nro_Cert=:Nro_Cert And c.Ide_Con=:Ide_Con And Numero In (" + Lst_Cont + ")"
        Me.CrearComando(querystring)
        AsignarParametroCadena(":Ide_Con", Ide_Con)
        AsignarParametroCadena(":Nro_Cert", Nro_Cert)
        Dim dataTb As DataTable = EjecutarConsultaDataTable()
