...
Public Class PdfManipulation2
    ...
    ''' <remarks></remarks>
    Public Shared Function RemoveRestrictions(ByVal restrictedPdf As String, Optional ByVal saveABackup As Boolean = True) As Boolean
    ...
    Public Overloads Shared Sub ExtractPdfPage(ByVal sourcePdf As String, ByVal paginaInicial As Integer, ByVal paginaFinal As Integer, ByVal outPdf As String)
        Dim reader As iTextSharp.text.pdf.PdfReader = Nothing
        Dim doc As iTextSharp.text.Document = Nothing
        Dim pdfCpy As iTextSharp.text.pdf.PdfCopy = Nothing
        Dim page As iTextSharp.text.pdf.PdfImportedPage = Nothing
        Try
            ...
            doc = New iTextSharp.text.Document(reader.GetPageSizeWithRotation(1))
            pdfCpy = New iTextSharp.text.pdf.PdfCopy(doc, New IO.FileStream(outPdf, IO.FileMode.Create))
            'pdfCpy= New iTextSharp.text.pdf.PdfCopy(doc,
            doc.Open()
            For pageNum As Integer = paginaInicial To paginaFinal
                page = pdfCpy.GetImportedPage(reader, pageNum)
                pdfCpy.AddPage(page)
            Next
            doc.Close()
    ...
    ''' in pageNumbersToExtract parameter.</remarks>
    ''' MODIFIDACA POR BORIS GONZALEZ
    Public Overloads Shared Function ExtractPdfPage(ByVal sourcePdf As Byte(), ByVal paginaInicial As Integer, ByVal paginaFinal As Integer) As Byte()
        ...
        reader = New iTextSharp.text.pdf.PdfReader(sourcePdf)
        doc = New iTextSharp.text.Document(reader.GetPageSizeWithRotation(1))
        Dim MemPdf As New MemoryStream
        pdfCpy = New iTextSharp.text.pdf.PdfCopy(doc, MemPdf)
        'pdfCpy= New iTextSharp.text.pdf.PdfCopy(doc,
        doc.Open()
        For pageNum As Integer = paginaInicial To paginaFinal
            page = pdfCpy.GetImportedPage(reader, pageNum)
