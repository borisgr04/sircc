...
Partial Class Procesos_GProcesoN2_Default
    ...
    Protected Sub CboTipPago_SelectedIndexChanged(sender As Object, e As EventArgs) Handles CboTipPago.SelectedIndexChanged
        If CboTipPago.Text = "PM" Then
            TxtCondicionPago.Text = DescripcionValorMensual()
        End If
    End Sub

    Function DescripcionValorMensual() As String
        Dim plazo1 As Integer = TxtPlazo.Text
        Dim plazo2 As Integer = TxtPlazo2.Text
        Dim Tplazo1 As String = CboTPlazo.Text
        Dim Tplazo2 As String = CboTPlazo3.Text
        Dim ValorAPagar As String = TxtValProp.Text
        Dim ValorMes As Decimal = 0
        Dim ValorDia As Decimal = 0
        Dim ValorFinal As Decimal = 0
        Dim totalDias As Integer = 0
        Dim Cadena As String = ""
        If plazo2 = 0 Then
            If Tplazo1 = "M" Then
                ValorMes = ValorAPagar / plazo1
                Cadena = String.Format(" en {0} mensualidades vencidas por valor de {1} ", plazo1, FormatCurrency(ValorMes))
            End If
        Else
            If Tplazo1 = "M" Then
                totalDias = plazo1 * 30 + plazo2
                ValorDia = ValorAPagar / totalDias
                ValorMes = Math.Round(ValorDia * 30)
                ValorFinal = Math.Round(ValorAPagar - ValorMes * plazo1)
                Cadena = String.Format(" en {0} mensualidades vencidas por valor de {1} y un pago final por valor de {2} ", plazo1, FormatCurrency(ValorMes), FormatCurrency(ValorFinal))
            End If
        End If
        TxtValPago.Text = 100
        Return Cadena
    ...
    Protected Sub BtnGenFP_Click(sender As Object, e As EventArgs) Handles BtnGenFP.Click
        If CboTipPago.Text = "PM" Then
            TxtCondicionPago.Text = DescripcionValorMensual()
            UpdFpago.Update()
        End If
